    ...
    <>
      <nav className="fixed top-0 w-full">
        ...
        <div className={
          ...
          {menu.map(link =>
            <>
              <a href={link.href} className="lg:p-2.5 lg:w-fit px-6 w-full flex flex-row gap-[5px] items-center group"
                onMouseEnter={() => setMenuHover(link.key)}
                // onMouseLeave={() => setMenuHover("")}
                onFocus={() => setMenuFocus(link.key)}
                onBlur={() => setMenuFocus(null)}
                onClick={() => menuActive == link.key ? setMenuActive(null) : setMenuActive(link.key)}
              >
                <span className="uppercase text-base/5 font-body group-hover:underline group-focus:underline underline-offset-6 p-2.5">{link.heading}</span>
              ...
              <ul className={
                ...
                {link.sublinks.map(sublink =>
                  <li className="p-2.5"><a href={sublink.href}>{sublink.name}</a></li>
                )}
              </ul>
            </>
          )}
      ...
      <nav className={
        "lg:flex bg-white justify-center gap-12.5 px-12.5 fixed absolute top-[60px] w-full hidden"
        + ((menuHover || menuFocus) ? " py-2" : "")
      }
        onMouseLeave={() => setMenuHover("")}
      >
        {(menuHover || menuFocus) && menu[handleHoverActive() - 1].sublinks.map(link =>
          <a href={link.href} className="hover:underline underline-offset-5 text-primary font-body p-2.5">{link.name}</a>
        )}
      </nav>
      <div className={
        "fixed bg-white flex items-center justify-between absolute top-[60px] w-full px-12.5 py-4 gap-4"
        + (searchShow ? "" : " hidden")
      }
      >
        <input className="font-body text-primary text-base/5 py-2.5 focus:outline-none w-full" type="text" placeholder="Search product..." />
